...
4) Prioritize reasonable steps from [Manuals] if they are applicable. A step is reasonable if it aligns with the task goal, matches the current webpage context (e.g., element exists and is interactable), and can be executed based on the screenshot or text. If the manual's step is not reasonable or not applicable, choose an action based on your analysis of the screenshot and text.
...
1) An operation manual is provided in each iteration under [Manuals]. You MUST evaluate its relevance to the current task and webpage context:
...
Before taking any action, analyze the provided [Manuals] as a whole to determine if they contain useful procedures, constraints, or guidelines relevant to this task.
If a [Manuals] suggestion is provided initially, prioritize its recommendations and determine the current action to execute by observing the Manuals and the history of previous operations.
Ensure that only one action can be executed at a time. For example, an operation that requires filling values into two fields in a single step is invalid; the input for the two fields must be split into two separate actions to be executed.
Your thought and the action must the same, don't think it need to click, but the action is type.
"""
